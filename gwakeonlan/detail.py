...
class DetailWindow(object):
  def __init__(self, winParent, show = False):
    # Load interface UI
    builder = Gtk.Builder()
    builder.add_from_file(os.path.join(DIR_UI, 'detail.glade'))
  ...
  def show(self):
    ...
    self.dialog.set_title(self.get_mac_address() and _('Edit machine') or _('Add machine'))
    while not response:
      ...
      if response == Gtk.ResponseType.OK:
        ...
        if err_msg:
          ...
          self.lblError.set_markup('<span foreground="red"><b>%s</b></span>' % err_msg)
